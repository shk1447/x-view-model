...
var PropertyHandler = /** @class */ (function (_super) {
    ...
    function PropertyHandler(init_property, options, devTools) {
        var _this = this;
        var _a, _b;
        _this = _super.call(this) || this;
        _this.middlewares = [];
        _this._sendHistory = [];
        _this._componentName = "Unknown";
...
var useInterfaceHandle = function (keys, handle) {
    var _a = react.useState(0); _a[0]; var setRenderCount = _a[1];
    var changeState = react.useCallback(function (args) {
...
var DevToolsHandler = /** @class */ (function () {
    function DevToolsHandler() {
        this.state = {
            components: new Map(),
        };
    }
    DevToolsHandler.prototype.components = function () {
        return this.state.components;
    };
    // 컴포넌트 등록 - 경로 매개변수 추가
    DevToolsHandler.prototype.registerComponent = function (context, componentName, componentPaths) {
        var _a;
        if (componentPaths === void 0) { componentPaths = []; }
        if (this.state.components.has(context)) {
            var components = this.state.components.get(context);
...
var devTools = new DevToolsHandler();
...
var getComponentInfo = function () {
    ...
    try {
        ...
        for (var i = 2; i < Math.min(stackLines.length, 8); i++) {
            ...
            for (var _i = 0, componentPatterns_1 = componentPatterns; _i < componentPatterns_1.length; _i++) {
                ...
                if (match && match[1]) {
                    var pathMatch = line.match(/\((.+?):\d+:\d+\)/);
                    if (pathMatch && pathMatch[1]) {
                        var fullPath = pathMatch[1];
                        // 경로에서 파일 시스템 경로 부분만 추출 (file:/// 같은 프로토콜 제거)
                        var normalizedPath = fullPath.replace(/^(file:\/\/\/|\/)[A-Za-z]:\//, "");
                        // 경로를 폴더 이름으로 분리
                        ...
                        paths.push.apply(paths, pathParts);
                    }
        ...
        if (componentName === "Anonymous") {
            var fileNameMatch = stack.match(/\/([A-Z][A-Za-z0-9_$]*)\.(jsx|tsx|js|ts)/);
...
var useViewModel = function (vm, keys, componentInfo) {
    if (componentInfo) {
        componentInfo = componentInfo;
    }
    else {
        componentInfo = react.useMemo(function () {
            return getComponentInfo();
        }, []);
    }
    devTools.registerComponent(vm.context, componentInfo.name, componentInfo.paths);
    var state = useInterfaceHandle(keys, vm.context);
    return [
        state,
        vm.context.send.bind(Object.assign(vm.context, {
...
var useComputedViewModel = function (vm, selector, keys) {
    var componentInfo = react.useMemo(function () {
        return getComponentInfo();
    }, []);
    var _a = useViewModel(vm, keys, componentInfo), state = _a[0], send = _a[1], controller = _a[2];
    var selectedState = react.useMemo(function () { return selector(state); }, [state, selector]);
...
var useMemoizedViewModel = function (vm, keys) {
    ...
    var _a = useViewModel(vm, keys, componentInfo), fullState = _a[0], send = _a[1], ref = _a[2];
    return [fullState, send, ref];
};

exports.DevToolsHandler = DevToolsHandler;
