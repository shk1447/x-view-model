...
class FlowHanlder extends EventHandler {
    constructor(flow, handler) {
        ...
        this.send = async (target, prev, err) => {
...
class PropertyHandler extends EventHandler {
    ...
    async send(name, payload, options) {
        if (options && options.sync) {
            try {
                const res = await this.property[name].apply(this.state, [
                    payload,
                ]);
                if (options.callback) {
...
const useViewModel = (vm, keys) => {
    ...
    const send = async (name, payload, options) => {
        if (options && options.sync) {
            try {
                const res = await vm.context.property[name].apply(vm.context.state, [payload]);
                if (options.callback) {
                    options.callback(res);
                }
                return true;
