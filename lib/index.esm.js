...
class FlowHanlder extends EventHandler {
    constructor(flow, handler) {
        super();
        this.onCurrentChange = (fn) => {
            this._fns.push(fn);
        };
        this.offCurrentChange = (fn) => {
            this._fns.splice(this._fns.indexOf(fn), 1);
        };
        this.send = (target, prev, err) => __awaiter(this, void 0, void 0, function* () {
            const pick = this._flow[target.replace("#", "")];
            if (pick) {
                try {
                    yield pick.invoke(this._handler.state, prev, err);
                    this.current = target.replace("#", "");
                    if (pick.onDone) {
                        if (typeof pick.onDone == "string") {
                            yield this.send(pick.onDone);
                        }
                        else {
                            const _target = yield pick.onDone(this._handler.state, prev, err);
                            if (_target) {
                                yield this.send(_target, target);
                            }
                        }
                    }
                }
                    ...
                    if (pick.onError) {
                        if (typeof pick.onError == "string") {
                            yield this.send(pick.onError, error);
                        }
                        else {
                            const _target = yield pick.onError(this._handler.state, prev, err);
                            if (_target) {
                                yield this.send(_target, prev, error);
                            }
                        }
                    }
                    return false;
...
const registViewFlow = (data, flow, options, ref) => {
    const handler = new PropertyHandler(data, options);
    const fHandler = new FlowHanlder(flow, handler);
    const vm = {
        context: handler,
        flow: fHandler,
        ref: ref,
...
const useViewModel = (vm, keys) => {
    ...
    const send = (name, payload, options) => __awaiter(void 0, void 0, void 0, function* () {
        if (options && options.sync) {
            try {
...
const useSelectedViewModel = (vm, selector, keys) => {
...
const useMemoizedViewModel = (vm, keys) => {
    ...
    const memoizedState = useMemo(() => {
        const partialState = {};
        keys.forEach((key) => {
            const value = get(fullState, key);
            set(partialState, key, value);
        });
        return partialState;
    }, [fullState, keys]);
...
export { PropertyHandler, registViewFlow, registViewModel, useMemoizedViewModel, useSelectedViewModel, useViewFlow, useViewModel };
